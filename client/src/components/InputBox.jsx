...
const InputBox = () => {
  const [state,dispatch,createNote,getNotes]=useContext(NotesContext);
    ...
    <Box onClick={showInputs} position="relative" width="680px" height="300px" left="300px">
      ...
      <Box display="flex" gap="15px" padding="2px 3px" marginTop={formData.image !== "" ? "20px" : "0px"} >
        <Box width="595px" wordBreak="break-all" >
          {isClicked ? <Input focusBorderColor={formData.background === "" ? "white" : `${formData.background}`} outline="none" border="none" onChange={handleFormData} type="text" name="title" value={formData.title} placeholder="Enter Title of the note" required/> : null }
          {isClicked ? <Input focusBorderColor={formData.background === "" ? "white" : `${formData.background}`} outline="none" border="none" onChange={handleFormData} type="text" name="tagline" value={formData.tagline} placeholder="Enter Tagline..." required/> : null }
          <Input focusBorderColor={formData.background === "" ? "white" : `${formData.background}`} border="none" outline="none" onChange={handleFormData} type="text" name="text" value={formData.text} placeholder="Take a note..." required/>
        </Box>
        <Box marginTop="5px">
